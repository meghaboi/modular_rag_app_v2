...
from typing import List, Tuple  # Removed Dict, Any
import numpy as np
from ..enums import VectorStoreType  # Relative import for enums
import logging

# import os # Removed os as it's unused at module level
...
class FAISSVectorStore(VectorStore):
    ...
    def search(
        ...
        distances, indices = self._index.search(
            query_np, min(top_k, len(self._documents))
        )
...
class ChromaVectorStore(VectorStore):
    ...
    def add_documents(
        ...
        self._collection.add(embeddings=embeddings, documents=documents, ids=ids)
    ...
    def _add_documents_milvus(
        ...
        try:
            ...
            try:
                ...
                data = [documents, embeddings]  # "text" field  # "embedding" field
...
class HybridVectorStore(VectorStore):
    ...
    def __init__(self, alpha: float = 0.5):
        ...
        from .search_strategies import (
            HybridSearch,
        )  # HybridSearch is now in search_strategies
    ...
    def search(
        self, query_embedding: List[float], top_k: int = 5, query: str = None
    ) -> List[Tuple[str, float]]:
        """
...
class VectorStoreFactory:
    ...
    def create_store(store_name: str, **kwargs) -> "VectorStore":
        ...
        # Classes are defined in this file, so direct instantiation is possible.
        # No need for 'from . import X' if X is in the same file.
