...
from typing import Dict, Any, Optional, Iterator, Tuple  # Removed List, Callable
import os
...
from ..enums import LLMModelType  # Relative import for enums
...
class StreamingLLM(ABC):
    ...
    @abstractmethod
    def generate(
        self, prompt: str, context: Optional[str] = None, evaluation_mode: bool = False
    ...
    def _set_last_call_usage(self, usage_data: Optional[Any], provider: str):
        ...
        if provider == "openai":
            ...
            normalized_usage = {
                ...
                + usage_data.output_tokens,  # Anthropic usage might not have total_tokens directly
            }
...
class ClaudeLLM(StreamingLLM):
    ...
    def generate(
        ...
        if system_prompt_to_use is not None:
            ...
            ]  # Pass as a list containing the string
