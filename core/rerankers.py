...
from typing import List, Tuple, Optional  # Removed Dict, Any
import os
import logging
from ..enums import RerankerModelType  # Relative import for enums
import requests
from .llm_models import StreamingLLM  # Relative import for llm_models
from .llm_models import ClaudeLLM  # Relative import for llm_models
import json
            ...
            if reranking_output and hasattr(reranking_output, "results"):
                for result_item in reranking_output.results:
                    ...
                    reranked_docs.append(
                        (result_item.document, result_item.relevance_score)
                    )
            else:
                logging.warning(
                    "Voyage reranker did not return expected 'results' attribute. Falling back to original documents."
                )
                # Fallback to original documents with a neutral score if API response is not as expected
                return [
                    (doc, 0.0) for doc in documents
                ]  # Changed score to 0.0 for neutral fallback

            return reranked_docs

        except Exception as e:
            logging.error(
                f"Error in reranking with Voyage: {str(e)}", exc_info=True
...
class LLMReranker(Reranker):
    ...
    def __init__(
        self, llm_client: StreamingLLM, model_name: str = "claude-3-7-sonnet-20240708"
    ):
    ...
    def rerank(self, query: str, documents: List[str]) -> List[Tuple[str, float]]:
        ...
        try:
            ...
            response_text = self._llm_client.generate(
                prompt, context="", model_name=self._model_name
            )  # Pass model_name
            ...
            try:
                ...
                if "```json" in response_text:
                    ...
                    "```" in response_text
                ):  # If no "json" tag, but still has backticks
            ...
            for item in reranked_data:
                if (
                    ...
                    and "document_index" in item
                    and "relevance_score" in item
                ):
                    original_index = item["document_index"]
                    if isinstance(original_index, int) and 0 <= original_index < len(
                        documents
                    ):
                        if (
                            original_index not in seen_indices
                        ):  # Ensure each document is added once
                            reranked_docs.append(
                                (
                                    documents[original_index],
                            ...
                            print(
                                f"Warning: Duplicate document_index {original_index} in LLM response. Skipping."
                            )
                    else:
                        print(
                            f"Warning: Invalid document_index {original_index} in LLM response. Skipping."
                        )
                else:
            ...
            if len(reranked_docs) < len(documents):
                print(
                    "Warning: LLM reranker did not return all documents. Appending missing ones with low score."
                )
                for i, doc in enumerate(documents):
                    if i not in seen_indices:
                        reranked_docs.append(
                            (doc, 0.1)
                        )  # Assign a low score for missing docs
...
class RerankerFactory:
    ...
    @staticmethod
    def create_reranker(
        reranker_name: str, llm_client: Optional[StreamingLLM] = None
    ) -> Reranker:
        ...
        if reranker_name == RerankerModelType.COHERE_V2:
            ...
            if not llm_client:
                ...
                llm_client = ClaudeLLM(
                    model_name="claude-3-7-sonnet-20240708"
                )  # Use ClaudeLLM
