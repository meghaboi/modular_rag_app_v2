...
from ..enums import EmbeddingModelType  # Relative import for enums
import requests
...
class MistralEmbedding(EmbeddingModel):
    ...
    def _batch_texts(
        self, texts: List[str], max_tokens_per_batch: int = 8192
    ) -> List[List[str]]:
        ...
        while True:
            try:
                response = self._client.embeddings.create(
                    model=self._model_name, inputs=inputs
                )
                ...
                if "429" in error_message and "rate limit" in error_message.lower():
                    ...
                    if retry_count > self._max_retries:
                        logger.error(
                            f"Maximum retries ({self._max_retries}) exceeded. Giving up."
                        )
                        raise e
                    ...
                    logger.warning(
                        f"Rate limit exceeded. Retrying in {actual_delay:.2f} seconds (retry {retry_count}/{self._max_retries})"
                    )
...
class VoyageEmbedding(EmbeddingModel):
    ...
    def _call_api_with_backoff(self, texts: List[str]) -> List[List[float]]:
        ...
        while True:
            try:
                ...
                if retry_count >= self._max_retries:
                    logger.error(
                        f"Failed to get embeddings after {retry_count} retries: {str(e)}"
                    )
                    raise e
                ...
                logger.warning(
                    f"API call failed: {str(e)}. Retrying in {actual_delay:.2f} seconds (retry {retry_count}/{self._max_retries})"
                )
                time.sleep(actual_delay)
                delay = min(delay * 2, self._max_delay)
    ...
    def _batch_documents(self, documents: List[str]) -> List[List[str]]:
        ...
        return [
            documents[i : i + self._batch_size]
