...
class RAGEvaluators:
    ...
    def answer_relevance_evaluator(run, reference=None) -> EvaluationResult:
        ...
        if (
            reference
            and reference.get("outputs")
            and "ground_truth" in reference.get("outputs", {})
        ):
        ...
        if ground_truth:
            ...
            eval_response = chain.invoke({"query": query, "response": response})
    ...
    def context_relevance_evaluator(run, reference=None) -> EvaluationResult:
        ...
        # Get evaluation
        eval_response = chain.invoke({"query": query, "contexts": context_text})
