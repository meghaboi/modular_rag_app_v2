...
# from typing import List, Dict, Any # Removed List, Dict, Any (not used)
...
from ..utils.subject_handler import update_rag_configuration  # Corrected path

# from ..utils.pipeline_utils import initialize_pipeline  # Removed: Not used in this file
from ..utils.enums import (  # Corrected path
    EmbeddingModelType,
    RerankerModelType,
    LLMModelType,
    VectorStoreType,
...
)  # Corrected path for run_all_permutations

# from ..utils.token_utils import TokenCostManager  # Removed: Not used in this file
...
def display_chat_interface():
    ...
    st.markdown(
        "Hey! Got questions about your textbook? Lay 'em on me. I'll break it down for ya."
    )

    if not st.session_state.messages:
        welcome_msg = (
            "Alright, let's get this study session started! What's on your mind?"
        )
                    ...
                    if tab_text:
                        ...
                        with tab_audio:
                            if audio_data:
                                ...
                                st.info(
                                    "Audio playback is not available for this message."
                                )

                    if elapsed_time is not None:
                        st.write(
                            f"_(JEFF cooked that up in {elapsed_time:.2f} seconds)_"
                        )
    ...
    if user_query:
        ...
        if st.session_state.pipeline:  # Ensure pipeline exists
            logging.info(
                f"Attempting to update RAG configuration for query: {user_query[:100]}..."
            )
            ...
            if config_update_status is False:  # Explicitly check for False (failure)
                ...
                logging.error(
                    "update_rag_configuration returned False, indicating a failure during re-initialization."
                )
            elif config_update_status is True:
                st.toast("✨ Smartly adjusted RAG settings for your query!")
                logging.info(
                    "update_rag_configuration returned True. Pipeline re-initialized."
                )
            else:  # config_update_status is None
                logging.info(
                    "update_rag_configuration returned None. No changes to RAG configuration were necessary."
                )
        # ============================================
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                with tab_stream_audio:
                    ...
                    audio_placeholder.info(
                        "Audio will be available when response is complete."
                    )
                ...
                logging.error(
                    f"Error processing query or generating audio: {e}", exc_info=True
                )
...
def display_evaluation_interface():
    ...
    st.markdown(
        "Let's test out different setups. Give me a question and the perfect answer (ground truth) to see how well various RAG configurations perform."
    )
    ...
    with col1:
        ...
        ground_truth = st.text_area(
            "Enter the ideal 'ground truth' answer:",
            height=100,
            key="eval_ground_truth",
        )
        st.info("Providing ground truth enables detailed RAGAS evaluation scores.")

        disable_eval_buttons = (
            st.session_state.pipeline is None or not st.session_state.file_path
        )
        process_button = st.button(
            "Evaluate Current Config", disabled=disable_eval_buttons
        )
        permutation_button = st.button(
            "Run All Permutations Test", disabled=disable_eval_buttons
        )
    ...
    if process_button and user_query:
        if not st.session_state.file_path:
            ...
            try:
                ...
                reranker_enum = RerankerModelType.from_string(st.session_state.reranker)
                llm_enum = LLMModelType.from_string(st.session_state.llm_model)
                chunking_strategy_enum = ChunkingStrategyType.from_string(
            ...
            missing_keys = check_api_keys(
                embedding_model_enum, vector_store_enum, reranker_enum, llm_enum
            )
            if missing_keys:
                ...
                with st.spinner("Evaluating current configuration..."):
                    ...
                    try:
                        print(user_query)
                        response, contexts, metrics = (
                            st.session_state.pipeline.process_query(user_query)
                        )
                        ...
                        if ground_truth:
                            try:
                                evaluation_results = (
                                    st.session_state.pipeline.evaluate_response(
                                        query=user_query,
                                        response=response,
                                        contexts=contexts,
                                ...
                                if evaluation_results and isinstance(
                                    ...
                                    # if valid_scores: # F841: avg_score was unused
                                    #     avg_score = sum(valid_scores) / len(
                                    #         valid_scores
                                    #     )
                                logging.info(
                                    f"Single config evaluation scores: {evaluation_results}"
                                )
                            except Exception as eval_e:
                                logging.error(
                                    f"Evaluation failed for single config run: {eval_e}",
                                    exc_info=True,
                                )
                            ...
                            logging.warning(
                                "No ground truth provided, skipping evaluation metrics."
                            )
                        ...
                        with col2:
                            ...
                            st.markdown(
                                f"**Configuration:** `{st.session_state.embedding_model} | {st.session_state.vector_store} | {st.session_state.reranker} | {st.session_state.llm_model} | {st.session_state.chunking_strategy}`"
                            )
                            ...
                            with summary_metric_cols[0]:
                                st.metric(
                                    ...
                                    value=f"{eval_elapsed_time:.2f}s",
                                )
                            if metrics:
                                ...
                                with summary_metric_cols[3]:
                                    ...
                                    if (
                                        evaluation_results
                                        and isinstance(evaluation_results, dict)
                                        and "llm_cost" in evaluation_results
                                    ):
                                    ...
                                    if cost is not None and isinstance(
                                        cost, (float, int)
                                    ):
                                        cost_display_value = f"${cost:.4f}"
                                    elif (
                                        cost is None and metrics
                                    ):  # If cost is explicitly None but metrics were processed
                                        cost_display_value = "$0.0000"  # Or indicate it was processed but no cost applicable

                                    st.metric(
                                        label="LLM Cost", value=cost_display_value
                                    )
                            ...
                            if (
                                evaluation_results
                                and isinstance(evaluation_results, dict)
                                and "error" not in evaluation_results
                            ):
                                ...
                                if evaluation_results:
                                    ...
                                    for i_loop_var in range(
                                        0, num_metrics, metrics_per_row
                                    ):
                                        current_row_metrics_data = metrics_list[
                                            i_loop_var : i_loop_var + metrics_per_row
                                        ]
                                        # Create columns for the current row of metrics
                                        score_display_cols = st.columns(
                                            len(current_row_metrics_data)
                                        )  # Will be 1 or 2 columns
                                        for k_loop_var, (
                                            metric_name,
                                            metric_value,
                                        ) in enumerate(current_row_metrics_data):
                                            with score_display_cols[k_loop_var]:
                                                ...
                                                st.metric(
                                                    ...
                                                    value=score_val_display,
                                                )
                                else:
                                    st.info("No scores generated.")
                            elif "error" in evaluation_results:
                                st.warning(
                                    f"Scores not calculated: {evaluation_results['error']}"
                                )
                            elif ground_truth:
                                st.warning("Scores could not be calculated.")
                            else:
                                st.info("Provide ground truth to see scores.")

                    except Exception as e:
                        logging.error(
                            f"Error running single evaluation: {e}", exc_info=True
                        )
        ...
        if not st.session_state.file_path:
            ...
            for e_perm, r_perm, l_perm in itertools.product(
                perm_emb, perm_rerank, perm_llm
            ):
                potential_missing.update(
                    check_api_keys(e_perm, VectorStoreType.FAISS, r_perm, l_perm)
                )
            if potential_missing:
                st.warning(
                    f"Heads up! Missing potential keys: {', '.join(potential_missing)}. Some permutations might fail."
                )
            else:
    ...
    with col2:
        if (
            st.session_state.permutation_df is not None
            and not st.session_state.permutation_df.empty
        ):
            st.subheader("Permutation Results Summary")
            st.markdown(
                f"Results have been saved to `{output_csv_filename}` in the application's root directory."
            )
            st.markdown(
                get_csv_download_link(
                    st.session_state.permutation_df,
            ...
            results_to_display["avg_score_numeric"] = results_to_display[
                "avg_score"
            ].fillna(-1)
            top_results = results_to_display.sort_values(
                "avg_score_numeric", ascending=False
            ).head(10)
            ...
            metric_cols_exist = sorted(
                [col for col in results_to_display.columns if col.startswith("metric_")]
            )
            ...
            if st.session_state.permutation_results:
                for index, row_data in enumerate(st.session_state.permutation_results):
                    label = (
                        f"{index}: {row_data.get('embedding_model','?')} / {row_data.get('vector_store','?')} / "
                ...
                if config_labels:
                    selected_index = st.selectbox(
                        ...
                        options=range(len(config_labels)),
                        format_func=lambda index: config_labels[index],
                        key="permutation_select",
                    )
                    if selected_index is not None and selected_index < len(
                        st.session_state.permutation_results
                    ):
                        selected_result = st.session_state.permutation_results[
                            selected_index
                        ]
                        st.markdown(f"**Details for Configuration {selected_index}:**")
                        st.markdown(
                            f"**Models:** `{selected_result.get('embedding_model','N/A')} | {selected_result.get('vector_store','N/A')} | {selected_result.get('reranker','N/A')} | {selected_result.get('llm_model','N/A')}`"
                        )
                        st.markdown(
                            f"**Chunking:** `{selected_result.get('chunking_strategy','N/A')}`"
                        )
                        st.write(
                            f"**Processing Time:** {selected_result.get('elapsed_time', 0):.2f} seconds"
                        )
                        with st.expander("Response", expanded=True):
                        ...
                        if (
                            isinstance(eval_scores, dict)
                            and "error" not in eval_scores
                            and eval_scores
                        ):
                            st.subheader("Evaluation Scores")
                            scores_items = list(eval_scores.items())
                            metrics_per_row_detail = (
                                2  # Max 2 metrics per row for detailed view
                            )

                            for i_detail_loop in range(
                                0, len(scores_items), metrics_per_row_detail
                            ):
                                row_items = scores_items[
                                    i_detail_loop : i_detail_loop
                                    + metrics_per_row_detail
                                ]
                                cols_detail = st.columns(
                                    len(row_items)
                                )  # Will be 1 or 2 columns
                                for idx_detail, (metric, score) in enumerate(row_items):
                                    with cols_detail[idx_detail]:
                                        if (
                                            metric == "llm_cost"
                                        ):  # Check if the metric is "llm_cost"
                                            score_display = (
                                                f"${score:.4f}"
                            ...
                            st.warning("Scores not available (failed/skipped?).")
                        else:
                            st.info(
                                "Provide ground truth during permutation run for scores."
                            )
                else:
                    st.info("No permutation results available.")
