...
import logging
import os
from ..utils.subject_configs import (  # Corrected path
    SUBJECT_CONFIGS,
    get_subject_config,
...
from ..utils.pipeline_utils import initialize_pipeline  # Corrected path
...
from ..utils.utils import save_uploaded_file, check_api_keys  # Corrected path

# from ..core.embedding_models import EmbeddingModelFactory  # Removed: Not directly used here
# from ..core.vector_stores import VectorStoreFactory  # Removed: Not directly used here
# from ..core.rerankers import RerankerFactory  # Removed: Not directly used here
# from ..core.llm_models import LLMFactory  # Removed: Not directly used here
# from ..core.rag_pipeline import RAGPipeline  # Removed: Not directly used here
# from ..core.chunking import (
#     ChunkingStrategyFactory,
# )  # Removed: Not directly used here


def display_settings_panel():
    st.sidebar.image(
        "https://i.postimg.cc/DfLpxwZJ/Chat-GPT-Image-May-7-2025-11-10-13-AM.png",
        width=80,
    )
    ...
    uploaded_file = st.sidebar.file_uploader(
        "Upload .txt or .pdf file", type=["txt", "pdf"], key="file_uploader"
    )
    ...
    if st.session_state.mode == "evaluation":
        # IsInEvaluationMode = True # F841: Unused variable
        st.sidebar.header("🧪 Evaluation Config")
        st.sidebar.info(
            "Adjust settings for Evaluation Mode. Press 'Initialize JEFF' after changing."
        )
        config_expander_expanded = True
    else:
        # IsInEvaluationMode = False # F841: Unused variable
    ...
    with st.sidebar.expander(
        "RAG Configuration Details", expanded=config_expander_expanded
    ):
        # disable_widgets = st.session_state.mode == "chat" # F841: Unused variable
        ...
        if st.session_state.mode == "chat":
            ...
            st.session_state.embedding_model = st.selectbox(
                ...
                index=get_safe_index(
                    embedding_options, st.session_state.embedding_model
                ),
        ...
        if st.session_state.mode == "chat":
            st.sidebar.text(
                f"Re-ranker Model: {st.session_state.get('reranker', DEFAULT_RERANKER_MODEL.value)}"
            )
            st.sidebar.text(
                f"LLM Model: {st.session_state.get('llm_model', DEFAULT_LLM_MODEL.value)}"
            )
            st.sidebar.text(
                f"Vector Store: {st.session_state.get('vector_store', DEFAULT_VECTOR_STORE.value)}"
            )
            st.sidebar.text(
                f"Chunking Strategy: {st.session_state.get('chunking_strategy', DEFAULT_CHUNKING_STRATEGY.value)}"
            )
            st.sidebar.text(
                f"Chunk Size: {st.session_state.get('chunk_size', DEFAULT_CHUNK_SIZE)}"
            )
            st.sidebar.text(
                f"Chunk Overlap: {st.session_state.get('chunk_overlap', DEFAULT_CHUNK_OVERLAP)}"
            )
            ...
            st.session_state.vector_store = st.selectbox(
                ...
                index=get_safe_index(
                    vector_store_options, st.session_state.vector_store
                ),
                key="sb_vector_store",
            )
            st.session_state.chunking_strategy = st.selectbox(
                ...
                index=get_safe_index(
                    chunking_strategy_options, st.session_state.chunking_strategy
                ),
            ...
            st.session_state.top_k = st.slider(
                ...
                max_value=15,
                value=(
                    subject_config.top_k
                    if hasattr(subject_config, "top_k")
                    else DEFAULT_TOP_K
                ),
                step=1,
                key="sb_top_k",
            ...
            try:
                selected_vector_store_enum = VectorStoreType.from_string(
            ...
            if selected_vector_store_enum == VectorStoreType.HYBRID:
                ...
                st.session_state.hybrid_alpha = st.slider(
                    ...
                    1.0,
                    (
                        subject_config.hybrid_alpha
                        if hasattr(subject_config, "hybrid_alpha")
                        else DEFAULT_HYBRID_ALPHA
                    ),
                    0.05,
                    key="sb_hybrid_alpha",
        ...
        if st.session_state.pipeline and hasattr(
            st.session_state.pipeline, "last_evaluation_scores"
        ):
            st.markdown("---")
            ...
            if scores and isinstance(scores, dict):
                ...
                st.info(
                    "No evaluation scores available yet. Run an evaluation to see scores."
                )
    ...
    with st.sidebar.expander("🔑 API Key Status", expanded=False):
        ...
        if st.session_state.api_key_status:
            ...
            for key_name in sorted_key_names:
                ...
                st.markdown(
                    f"{icon} {key_name}: <span style='color:{color};'>{status}</span>",
    ...
    if st.sidebar.button(
        ...
        if missing_keys:
            st.sidebar.error(
                f"Cannot initialize. Missing keys: {', '.join(missing_keys)}", icon="🔑"
            )
        else:
            ...
            if pipeline_instance:
                ...
                st.session_state.permutation_results = None
                logging.info(
                    f"Pipeline initialized and set in session_state. Type: {type(st.session_state.pipeline)}. Triggering rerun."
                )
                st.rerun()
            else:
                st.session_state.pipeline = None
                ...
                logging.error(
                    "Pipeline initialization failed. initialize_pipeline returned None or Falsy."
                )
    elif disable_init:
