...
import openai  # Standard/Third-party
import streamlit as st  # Standard/Third-party
from .subject_configs import (  # Relative import
    SubjectConfig,
    get_subject_config,
    DEFAULT_CHUNK_SIZE,
...
from .pipeline_utils import initialize_pipeline  # Relative import
...
from .utils import determine_prompt_nature  # Relative import
...
def get_subject_configuration(subject: str, query: str) -> Dict[str, Any]:
    ...
    raise NotImplementedError(
        "This function is deprecated. Use get_config_by_prompt_nature instead."
    )
...
def get_config_by_prompt_nature(query: str) -> SubjectConfig:
    ...
    logging.info(
        f"Determined prompt nature: {prompt_nature} for query: '{query[:50]}...'"
    )

    config = get_subject_config(prompt_nature)
    logging.info(
        f"Using configuration for '{prompt_nature}': ChunkSize={config.chunk_size}, Overlap={config.chunk_overlap}, TopK={config.top_k}, Alpha={config.hybrid_alpha}"
    )

    return config


def update_rag_configuration(
    query: str, pipeline, subject: Optional[str] = None
) -> Optional[bool]:
    """
    ...
    try:
        ...
        if not nature_config:
            ...
            if subject:
                logging.info(
                    f"Falling back to subject-based configuration for: {subject}"
                )
                nature_config = get_subject_config(
                    subject
                )  # Use the original get_subject_config for subject fallback
            else:
                logging.warning(
        ...
        current_chunk_overlap = st.session_state.get(
            "chunk_overlap", DEFAULT_CHUNK_OVERLAP
        )
        current_top_k = st.session_state.get("top_k", DEFAULT_TOP_K)
        current_hybrid_alpha = st.session_state.get(
            "hybrid_alpha", DEFAULT_HYBRID_ALPHA
        )

        # Check if current configuration matches nature-derived config
        if (
            current_chunk_size == nature_config.chunk_size
            and current_chunk_overlap == nature_config.chunk_overlap
            and current_top_k == nature_config.top_k
            and current_hybrid_alpha == nature_config.hybrid_alpha
        ):
            logging.info(
        ...
        logging.info(
            f"Updating RAG parameters based on query nature. New config: ChunkSize={nature_config.chunk_size}, Overlap={nature_config.chunk_overlap}, TopK={nature_config.top_k}, Alpha={nature_config.hybrid_alpha} for query: '{query[:50]}...'"
        )
        ...
        try:
            embedding_enum = EmbeddingModelType.from_string(
            ...
            cs_enum = ChunkingStrategyType.from_string(
        ...
        if pipeline_instance:
            ...
            logging.info(
                f"Successfully re-initialized RAG pipeline based on query nature and updated session_state for query: '{query[:50]}...'"
            )
            return True
        else:
            logging.error(
        ...
        logging.error(
            f"Error updating RAG configuration for query '{query[:50]}...' (Subject: {log_subject}): {str(e)}",
            exc_info=True,
        )
        return False
