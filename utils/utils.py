...
def save_uploaded_file(uploaded_file):
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=file_suffix) as temp:
            ...
            temp_path = temp.name
            logging.info(
                f"Saved uploaded file '{uploaded_file.name}' to temporary path: {temp_path}"
            )
            ...
            if file_suffix.lower() == ".pdf":
                try:
                    ...
                    try:
                        os.unlink(temp_path)
                        logging.info(
                            f"Converted PDF to text and saved to: {text_temp_path}"
                        )
...
def check_api_keys(embedding_model_enum, vector_store_enum, reranker_enum, llm_enum):
    ...
    openai_needed = (
        ...
        or llm_enum in [LLMModelType.OPENAI_GPT35, LLMModelType.OPENAI_GPT4]
        or True
    )  # OpenAI TTS always needs it
    cohere_needed = (
        embedding_model_enum == EmbeddingModelType.COHERE
        or reranker_enum
        in [
            RerankerModelType.COHERE_V2,
            RerankerModelType.COHERE_V3,
            RerankerModelType.COHERE_MULTILINGUAL,
        ]
    )
    gemini_needed = (
        embedding_model_enum == EmbeddingModelType.GEMINI
        or llm_enum == LLMModelType.GEMINI
    )
    anthropic_needed = llm_enum in [
        LLMModelType.CLAUDE_3_OPUS,
        LLMModelType.CLAUDE_37_SONNET,
    ]
    mistral_needed = embedding_model_enum == EmbeddingModelType.MISTRAL or llm_enum in [
        LLMModelType.MISTRAL_LARGE,
        LLMModelType.MISTRAL_MEDIUM,
        LLMModelType.MISTRAL_SMALL,
    ]
    voyage_needed = (
        embedding_model_enum == EmbeddingModelType.VOYAGE
        or reranker_enum in [RerankerModelType.VOYAGE, RerankerModelType.VOYAGE_2]
    )
    ...
    if openai_needed:
        ...
        if not is_available:
            missing_keys_list.append(key_name)
...
def text_to_speech(text: str) -> Optional[bytes]:
    ...
    try:
        ...
        logging.info(
            f"Requesting OpenAI TTS: voice='{selected_voice}', model='{selected_model}', text length (cleaned): {len(cleaned_text)}"
        )
        ...
        logging.info(
            f"OpenAI TTS audio generated successfully ({len(audio_bytes)} bytes)."
        )
        return audio_bytes
...
def determine_prompt_nature(query: str) -> str:
    ...
    try:
        ...
        if tool_calls:
            ...
            logging.info(
                f"Prompt nature classification for query '{query}': Nature='{nature}', Confidence={confidence}"
            )

            if (
                nature in ALLOWED_NATURES
        ...
        logging.error(
            f"Error determining prompt nature for query '{query}': {e}", exc_info=True
        )
        return DEFAULT_NATURE
