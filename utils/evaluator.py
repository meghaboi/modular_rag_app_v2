...
# from langchain_google_genai import ChatGoogleGenerativeAI  # Removed: Not used
...
class BuiltinEvaluator(BaseEvaluator):
    ...
    def evaluate(
        ...
        for metric in self._metrics:
            if metric == "answer_relevance":
                results[metric] = self._evaluate_answer_relevance(
    ...
    def _evaluate_answer_relevance(
        ...
        if ground_truth:
            ...
            chain = prompt_template | self._evaluator_model
            response_obj = chain.invoke({"query": query, "response": response})
...
class RAGASEvaluator(BaseEvaluator):
    ...
    def __init__(self, metrics: List[str] = None):
        ...
        try:
            import ragas  # Used for ragas.llm and ragas.evaluate

            # from ragas.metrics import ( # These specific sub-module imports are unused
            #     faithfulness,
        ...
        if metrics is None:
            ...
            invalid_metrics = [
                m for m in metrics if m not in self._ragas_metrics and m != "f1_score"
            ]
            if invalid_metrics:
    ...
    def evaluate(
        ...
        try:
            ...
            # import ragas # Already imported or should be if used by ragas_evaluate
            from ragas import evaluate as ragas_evaluate

            # import pandas as pd # Removed: Not used
            ...
            if hasattr(results, "scores") and results.scores:
                ...
                if isinstance(results.scores, list) and len(results.scores) > 0:
                    scores_data = (
                        results.scores[0] if isinstance(results.scores[0], dict) else {}
                    )
                    for metric in self._metrics:
                        if metric in scores_data:
                            ...
                            for alt_name in self.metric_name_map.get(metric, []):
                                if alt_name in scores_data:
                                    ...
                                    metrics_dict[metric] = round(scaled_value, 2)
                                    logging.info(
                                        f"Found metric {metric} as {alt_name}, value: {raw_value}"
                                    )
                                    found = True
                                    break
                            ...
                            if not found:
                                logging.warning(
                                    f"Metric {metric} not found in scores dictionary"
                                )
            ...
            if "f1_score" in self._metrics:
                ...
                if recall > 0 and precision > 0:
                    ...
                    f1_score = (
                        2 * (recall_01 * precision_01) / (recall_01 + precision_01)
                    )
                    ...
                    metrics_dict["f1_score"] = (
                        3.0  # Default middle value if components missing
                    )
...
class DeepEvaluator(BaseEvaluator):
        ...
        for metric in self._metrics:
            try:
                if metric == "answer_relevance":
                    ...
                    results[metric] = self._evaluate_answer_consistency(
                        response, contexts
                    )
    ...
    def _extract_score_from_response(self, response_text: str) -> float:
        ...
        if (
            "excellent" in lower_text
            or "perfect" in lower_text
            or "completely" in lower_text
        ):
            return 5.0
        elif "good" in lower_text or "mostly" in lower_text:
            return 4.0
        elif (
            "moderate" in lower_text
            or "partial" in lower_text
            or "average" in lower_text
        ):
            return 3.0
        elif "poor" in lower_text or "slight" in lower_text:
            return 2.0
        elif (
            "terrible" in lower_text
            or "complete" in lower_text
            and "irrelevant" in lower_text
        ):
            return 1.0
...
class CustomEvaluator(BaseEvaluator):
    ...
    def __init__(self, metrics: List[str]):
        ...
        try:
            if not os.environ.get("ANTHROPIC_API_KEY"):
                raise ValueError(
                    "ANTHROPIC_API_KEY environment variable not set. This is required for CustomEvaluator with Claude."
                )
            ...
            raise ValueError(
                "langchain_anthropic is not installed. Please install it to use Claude for custom evaluation."
            )
        except Exception as e:
        ...
        if self._evaluator_model is None:
            raise ValueError(
                "Claude model (claude-3-opus-20240229) could not be initialized. Please check ANTHROPIC_API_KEY and langchain_anthropic installation."
            )

    def _parse_llm_response_to_list(
        self, response_content: str, item_type: str = "statement"
    ) -> List[str]:
        ...
        if (
            not items and response_content.strip()
        ):  # If parsing failed but there was content
            # Fallback: consider the whole response as a single item if it's not clearly a list
            if (
                len(response_content.strip().split()) > 5
            ):  # Heuristic: if it's a sentence/paragraph
                items.append(response_content.strip())
            else:  # Otherwise, might be a malformed list, try splitting by common delimiters
                items = [
                    i.strip() for i in re.split(r"[,;]", response_content) if i.strip()
                ]

        if not items and response_content.strip():
            print(
                f"Warning: Could not parse LLM response into a list of {item_type}s. Response: {response_content}"
            )
            return [
                response_content.strip()
            ]  # Return the whole response as a single item as a last resort
        return items

    def _parse_llm_yes_no_response(
        self, response_content: str, prompt_details: str
    ) -> bool:
        """Parses LLM response to a boolean for Yes/No questions."""
        response_lower = response_content.strip().lower()
        if (
            "yes" in response_lower and "no" not in response_lower
        ):  # Check for "yes" and not "yes, but no..."
            return True
            ...
            print(
                f"Warning: Ambiguous Yes/No response for prompt '{prompt_details}'. Response: {response_content}. Defaulting to False."
            )
            return False

    def _parse_llm_float_response(
        self, response_content: str, prompt_details: str
    ) -> float:
        ...
        try:
            ...
            if match:
                ...
                print(
                    f"Warning: Could not parse float from response for prompt '{prompt_details}'. Response: {response_content}. Defaulting to 0.0."
                )
                return 0.0
        except ValueError:
            print(
                f"Warning: ValueError parsing float for prompt '{prompt_details}'. Response: {response_content}. Defaulting to 0.0."
            )
            return 0.0
    ...
    def evaluate(
        ...
        for metric in self._metrics:
            if metric == EvaluationMetricType.CONTEXT_RECALL.value:
                ...
                results[metric] = self._evaluate_answer_correctness(
    ...
    def _evaluate_context_recall(
        ...
        if not self._evaluator_model:
            print(
                "Warning: Evaluator model not available for context_recall. Returning 0.0."
            )
            return 0.0
        ...
        try:
            response_statements = chain_statements.invoke(
                {"ground_truth": ground_truth}
            )
            ground_truth_statements = self._parse_llm_response_to_list(
                response_statements.content, "ground truth statement"
            )
        except Exception as e:
            print(
                f"Error getting statements from LLM for context_recall: {e}. Returning 0.0"
            )
        ...
        for stmt in ground_truth_statements:
            if not stmt.strip():  # Skip empty statements
                continue
            try:
                response_attribution = chain_attribution.invoke(
                    {"contexts": contexts_str, "statement": stmt}
                )
                if self._parse_llm_yes_no_response(
                    response_attribution.content,
                    f"context_recall attribution for statement: {stmt}",
                ):
                    attributed_statements += 1
            except Exception as e:
                print(
                    f"Error during attribution check for context_recall statement '{stmt}': {e}"
                )
                # Optionally, decide how to handle errors, e.g., assume not attributed

        # Step 3: Calculate context recall.
        recall_0_to_1 = (
            attributed_statements / len(ground_truth_statements)
            if ground_truth_statements
            else 0.0
        )
    ...
    def _evaluate_context_precision(
        ...
        if not self._evaluator_model:
            print(
                "Warning: Evaluator model not available for context_precision. Returning 0.0."
            )
            return 0.0
        ...
        for i, context_chunk in enumerate(contexts):
            ...
            if not context_chunk.strip():  # Skip empty context chunks
                precision_at_k = (
                    relevant_chunks_found / k
                )  # if chunk is empty, it means previous relevant count over k
            ...
            try:
                ...
                if self._parse_llm_yes_no_response(
                    response_relevance.content,
                    f"context_precision relevance for chunk {k}",
                ):
                    is_relevant = True
            except Exception as e:
                print(
                    f"Error during relevance check for context_precision chunk {k}: {e}"
                )
        ...
        scaled_precision = 1.0 + (
            min(max(mean_precision_0_to_1, 0.0), 1.0) * 4.0
        )  # Scale to 1-5
    ...
    def _evaluate_answer_relevancy(
        ...
        if not self._evaluator_model:
            print(
                "Warning: Evaluator model not available for answer_relevancy. Returning 0.0."
            )
            return 0.0
        ...
        generated_questions = []
        try:
            response_qgen = chain_qgen.invoke(
                {"answer": answer, "contexts": contexts_str}
            )
            generated_questions = self._parse_llm_response_to_list(
                response_qgen.content, "generated question"
            )
        except Exception as e:
        ...
        if not generated_questions:
            print(
                "Warning: No questions generated by LLM for answer_relevancy. Returning 0.0."
            )
            return 0.0
        ...
        for gen_q in generated_questions:
            if not gen_q.strip():  # Skip empty generated questions
                continue
            try:
                response_similarity = chain_similarity.invoke(
                    {"original_question": query, "generated_question": gen_q}
                )
                similarity_score = self._parse_llm_float_response(
                    response_similarity.content,
                    f"answer_relevancy similarity for gen_q: {gen_q}",
                )
                total_similarity += similarity_score
                actual_eval_count += 1
            except Exception as e:
                print(
                    f"Error calculating similarity for answer_relevancy gen_q '{gen_q}': {e}"
                )

        if actual_eval_count == 0:
            print(
                "Warning: Could not calculate similarity for any generated questions in answer_relevancy. Returning 1.0."
            )
            return 1.0  # Bottom of 1-5 scale

        mean_similarity_0_to_1 = total_similarity / actual_eval_count
        scaled_similarity = 1.0 + (
            min(max(mean_similarity_0_to_1, 0.0), 1.0) * 4.0
        )  # Scale to 1-5
    ...
    def _evaluate_faithfulness(self, answer: str, contexts: List[str]) -> float:
        ...
        if not self._evaluator_model:
            print(
                "Warning: Evaluator model not available for faithfulness. Returning 0.0."
            )
            return 0.0
        ...
        try:
            ...
            answer_statements = self._parse_llm_response_to_list(
                response_statements.content, "answer statement"
            )
        except Exception as e:
        ...
        if not answer_statements:
            print(
                "Warning: No statements parsed from answer for faithfulness. Returning 0.0."
            )
            return 0.0
        ...
        for stmt in answer_statements:
            if not stmt.strip():  # Skip empty statements
                continue
            try:
                response_inference = chain_inference.invoke(
                    {"contexts": contexts_str, "statement": stmt}
                )
                if self._parse_llm_yes_no_response(
                    response_inference.content,
                    f"faithfulness inference for statement: {stmt}",
                ):
                    faithful_statements += 1
            except Exception as e:
                print(
                    f"Error during inference check for faithfulness statement '{stmt}': {e}"
                )

        # Step 3: Calculate faithfulness.
        faithfulness_score_0_to_1 = (
            faithful_statements / len(answer_statements) if answer_statements else 0.0
        )
        scaled_faithfulness = 1.0 + (
            min(max(faithfulness_score_0_to_1, 0.0), 1.0) * 4.0
        )  # Scale to 1-5
    ...
    def _evaluate_answer_correctness(self, answer: str, ground_truth: str) -> float:
        ...
        if not self._evaluator_model:
            print(
                "Warning: Evaluator model not available for answer_correctness. Returning 0.0."
            )
            return 0.0
        ...
        try:
            response_factual = chain_factual.invoke(
                {"ground_truth": ground_truth, "answer": answer}
            )
            factual_analysis_json = json.loads(
                response_factual.content.strip()
            )  # Expecting JSON directly
            ...
            print(
                f"Error decoding JSON for factual analysis in answer_correctness: {e}. Response: {response_factual.content if 'response_factual' in locals() else 'N/A'}"
            )
        except Exception as e:
        ...
        try:
            response_semantic = chain_semantic.invoke(
                {"ground_truth": ground_truth, "answer": answer}
            )
            semantic_similarity = self._parse_llm_float_response(
                response_semantic.content, "answer_correctness semantic similarity"
            )
        except Exception as e:
        ...
        answer_correctness_score_0_to_1 = (factual_weight * factual_f1_score) + (
            semantic_weight * semantic_similarity
        )
        scaled_correctness = 1.0 + (
            min(max(answer_correctness_score_0_to_1, 0.0), 1.0) * 4.0
        )  # Scale to 1-5
...
class EvaluatorFactory:
    ...
    def create_evaluator(
        backend_type: EvaluationBackendType, metrics: List[str]
    ) -> BaseEvaluator:
...
# New function to encapsulate evaluation logic
def evaluate_rag_response(
    ...
    ground_truth: str,
    cost: Optional[float] = None,
    metrics_to_include: Optional[Dict[str, Any]] = None,
    backend_type: EvaluationBackendType = EvaluationBackendType.RAGAS_V2,  # Default backend
) -> Dict[str, Any]:
    """
    Evaluates a RAG response using a specified backend and merges performance metrics.
    ...
    Args:
        ...
        cost: Optional LLM cost for the generation.
        metrics_to_include: Optional dictionary of other performance metrics to include.
        backend_type: The evaluation backend to use.

    Returns:
        A dictionary containing evaluation scores and included performance metrics.
    """
    # This import will be correct once evaluator.py is in the utils/ directory
    from .enums import EvaluationMetricType

    metrics_for_backend = EvaluationMetricType.get_metrics_for_backend(backend_type)

    evaluator_instance = EvaluatorFactory.create_evaluator(
        backend_type=backend_type, metrics=metrics_for_backend
    )

    # The 'cost' is passed to the evaluator's evaluate method, which should handle adding it.
    evaluation_scores = evaluator_instance.evaluate(
        query=query,
        response=response,
        contexts=contexts,
        ground_truth=ground_truth,
    ...
    # Combine with other metrics if provided
    if metrics_to_include:
        final_scores = metrics_to_include.copy()
        # evaluation_scores should take precedence for any metrics it explicitly calculates (e.g. if it also calculates cost)
        final_scores.update(evaluation_scores)
        return final_scores
    else:
