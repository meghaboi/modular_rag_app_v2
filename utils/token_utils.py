...
class TokenCostManager:
    ...
    PRICING_DATA = {
        ...
        "gpt-3.5": {
            "input": 0.0005,
            "output": 0.0015,
        },  # $0.50/1M input, $1.50/1M output (e.g. gpt-3.5-turbo-0125)
        "gpt-3.5-16k": {
            "input": 0.003,
            "output": 0.004,
        },  # Older model, may be covered by gpt-3.5-turbo with startswith
        ...
        "claude-3-7-sonnet": {
            "input": 0.003,
            "output": 0.015,
        },  # Specific version used by factory
        # Google
    ...
    def calculate_cost(
        ...
        if not model_pricing:
            ...
            if not model_pricing:
                print(
                    f"Warning: Pricing not found for model '{model_name}' (normalized: '{normalized_model_name}'). Cost will be 0."
                )
                return 0.0
